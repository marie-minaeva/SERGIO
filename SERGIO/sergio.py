            ...
            if (shared_coop_state <= 0):
                for row in reader:
                    ...
                    for regId, K, C_state in zip(row[2: 2 + nRegs], row[2+nRegs : 2+2*nRegs], row[2+2*nRegs : 2+3*nRegs]):
                        currInteraction.append((np.int64(float(regId)), np.float64(K), np.float64(C_state), 0)) # last zero shows half-response, it is modified in another method
                        allRegs.append(np.int64(float(regId)))
                        currParents.append(np.int64(float(regId)))
                        self.graph_[np.int64(float(regId))]['targets'].append(np.int64(float(row[0])))

                    self.graph_[np.int64(float(row[0]))]['params'] = currInteraction
                    self.graph_[np.int64(float(row[0]))]['regs'] = currParents
                    self.graph_[np.int64(float(row[0]))]['level'] = -1 # will be modified later
                    allTargets.append(np.int64(float(row[0])))
